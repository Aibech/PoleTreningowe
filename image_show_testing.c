...
uint8_t bmp_buffer[1024 * 1024]; // bufor np. na 1MB obrazek

void* mmap(void *addr, size_t length, int prot, int flags, int fd, size_t offset) {
    // fd to deskryptor pliku, który zakładamy że jest już otwarty (np. z fopen/fdopen z systemu plików FatFS lub emulowanego)
    
    // Przesuwamy plik na offset (jeśli potrzebne)
    if (fseek((FILE*)fd, offset, SEEK_SET) != 0) {
        return MAP_FAILED;
    }

    // Czytamy dane do bufora
    size_t bytes_read = fread(bmp_buffer, 1, length, (FILE*)fd);
    if (bytes_read != length) {
        return MAP_FAILED;
    }

    return bmp_buffer; // Zwracamy wskaźnik do danych
}
